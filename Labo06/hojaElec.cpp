...
void HojaCalc::insNodo(void)
{
    ...
    do
    {
        ...
        if (fila >= 1 && fila <= totFil && columna >= 1 && columna <= totCol)
        {
            ...
            cout << "Hubicación no valida para nuevo dato" << endl
                 << endl;
...
void HojaCalc::sumarCol(int col){
   ...
   do{
       if(s->nCol == col){
           ...
           cout <<"\nSuma de la columna "<<col<<": "<< sum << endl;
           break;
       }
       s =s->sigCol;
   }while(s->nCol != 0);
...
{
    nodo *s;
    int totFilas, totColumnas, fil, col, i, f;
    ...
    cout<<"\nque fila desea sumar ";
    cin>>f;
    hojaCalculo.sumarFila(f);

    cout<<"\nque columna desea sumar ";
    cin>>f;
    hojaCalculo.sumarCol(f);
